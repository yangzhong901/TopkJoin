...
void dpjoin::PerformTopkDualJoin()
{	
	hightime htime;
	double totalpqtime=0.0;

	int k = Getk();
	int delta = fixedelta;//固定的delta值，例如delta=2
	Similarity sim(k);
	...
	while (!events.isEmpty())
	{
		...
		if (xpos + curdelta < xlen)//添加x的下一个event
		{			
	...
	cout << "DJ Candidates: " + to_string(candidateNum) << endl;
	cout << "DJ kth Result overlap: " + to_string(tmpresults.get().sim) << endl;
